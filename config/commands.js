...
var commands = exports.commands = {
	...
	givebucks: function(target, room, user) {
		...
		if (isNaN(parts[1])) {
			return this.sendReply('Please use a valid value.');
		}
	...
	removepoints: function(target, room, user) {
		...
		if (isNaN(parts[1])) {
			return this.sendReply('Please use a valid amount.');
		}
		var cleanedUp = parts[1].trim();
	...
	buy: function(target, room, user) {
		...
		if (target === 'symbol') {
			...
			if (price <= user.money) {
				...
				return this.sendReply('You do not have enough points for this. You need ' + (price - user.money) + ' more bucks to buy ' + target + '.');
			}
		}
		if (target === 'custom') {
			price = 20;
		...
		if (target === 'potd') {
			...
			if (price <= user.money) {
				user.money = user.money - price;
				this.sendReply('You have purchased the ability to set POTD. To do this, type /potd [pokemon name]. Make sure to spell right as you ony get one!');
				user.canPOTD = true;
		...
		if (target === 'client') {
			return this.sendReply('This item is currently unavailable and will be added in the future.')
		}
		if (match === true) {
			var re = new RegExp(line,"g");
			fs.readFile('config/money.csv', 'utf8', function (err,data) {
	...
	customsymbol: function(target, room, user) {
		...
		var a = target;
		if (a === "+" || a === "◈" || a === "$" || a === "%" || a === "@" || a === "&" || a === "~" || a === "#" || a === "a" || a === "b" || a === "c" || a === "d" || a === "e" || a === "f" || a === "g" || a === "h" || a === "i" || a === "j" || a === "k" || a === "l" || a === "m" || a === "n" || a === "o" || a === "p" || a === "q" || a === "r" || a === "s" || a === "t" || a === "u" || a === "v" || a === "w" || a === "x" || a === "y" || a === "z") {
	...
	shop: function(target, room, user) {
		if (!this.canBroadcast()) return;
		this.sendReplyBox('<center><h4><b><u>Pokemon Database Shop</u></b></h4><table border="1" cellspacing ="0" cellpadding="3"><tr><th>Command</th><th>Description</th><th>Cost</th></tr>' +
			...
			'<tr><td>Room</td><td>Buys a chatroom for you to own (within reason, can be refused if it\'s thought to be innapropriate)</td><td>100</td></tr>' +
			'<tr><td>Trainer</td><td>Buys a trainer card which shows information through a command  (note: third image costs 10 points extra, ask for more details)</td><td>30</td></tr>' +
			'<tr><td>POTD</td><td>Buys the ability to set the POTD (one time use)!</td><td>25</td></tr>' +
			...
			'</table><br />To buy an item from the shop, use /buy [command]. <br />Also do /moneycommands to view money based commands.<br />' +
			'If you want to update existing custom avatar or trainer card, ask Administartion (10 points to update).</center>');
	...
	removeitem: function(target, room, user) {
		...
		if (target === 'symbol') {
			...
			if (targetUser.canPOTD) {
				...
				this.sendReply(targetUser.name + ' no longer has a potd ready to use.');
	...
	moneycommands: function(target, room, user) {
		...
		return this.sendReplyBox('The command for the Money system:<br />' + 
			'/shop - Shows the shop with the items you can buy.<br />' + 
			...
			'/atm [username] - Show your point balance (if just /atm) or show someone else\'s point balance with (if /atm [username].<br />' );
