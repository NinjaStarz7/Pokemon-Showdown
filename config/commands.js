...
var commands = exports.commands = {
	...
	var data = fs.readFileSync('config/badges.csv','utf8')
		...
		if (match === true) {
			this.sendReplyBox(targetUser.name + ' has ' + numBadges + ' DB badge(s).');
		}
		if (match === false) {
			connection.sendTo(room, '' + targetUser.name + ' has no DB badges.');
		}
	...
	givebadge: function(target, room, user, connection) {
		...
		if (isNaN(parts[1])) {
			return this.sendReply('Please use a realistic number.');
		}
	var data = fs.readFileSync('config/badges.csv','utf8')
		var match = false;
		var numBadges = 0;
		...
		return this.sendReply(targetUser + ' was given ' + parts[1] + ' DB badges. ' + targetUser + ' now has ' + Users.get(targetUser.userid).badges + ' DB badges.');
		}
		}
		else {
